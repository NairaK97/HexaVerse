...
export default {
    setup() {
        ...
        const  designBlock = computed(() => store.designBlock)
    return {
     designBlock
    };
  },
...
<template>
    <div class="web-design-container">
        <div class="web-design-text">
            <h2>Stunning web designs that captivate your audience.</h2>
            <h1>Web Design</h1>
        </div>
        <div class="cols">
        <div class="col" v-for="item in designBlock" :key="item.text" ontouchstart="this.classList.toggle('hover');">
          <div class="container">
            <div class="front" :style="{ backgroundImage: 'url(' + item.image + ')' }">
              <div class="inner">
                <p>{{ item.text }}</p>
...
.wrapper {
    width: 90%; margin: 0 auto; max-width: 80rem;
    ;
}
.cols {
...
.front:after {
    ...
    display: block; opacity: .6; background-color: #000;
...
.inner {
    ...
    outline: 1px solid transparent; -webkit-perspective: inherit;
...
.front .inner p:after {
    content: ''; width: 4rem; height: 2px; position: absolute;
    background: #C6D4DF; display: block; left: 0; right: 0;
    margin: 0 auto; bottom: -.75rem;
}
.front .inner span {
    color: rgba(255, 255, 255, 0.7); font-family: 'Montserrat'; font-weight: 300;
}
